...
describe("create user", () => {
  ...
  it("should return status of 201 and the user created", async () => {
    ...
    // this is one way to test the saveMethod on new created user
    // expect(User.mock.instances[0].save).toHaveBeenCalled();
  });

  it("send status of 400 when user failed to save user", async () => {
    jest.spyOn(validator, "validationResult").mockImplementationOnce(() => ({
      isEmpty: jest.fn(() => true),
    }));
    const saveMethod = jest
      .spyOn(User.prototype, "save")
      .mockImplementationOnce(() => Promise.reject("Fail to save"));
    await createUserHandler(mockRequest, mockResponse);
    expect(saveMethod).toHaveBeenCalled();
